...
$(document).ready(function () {
  ...
  p.oncanplaythrough = function () {
    console.log("CANPLAY through"); // $("#playBtn").attr("disabled", false)
  };

  p.oncanplay = function () {
    console.log("CANPLAY"); // $("#playBtn").attr("disabled", false)
  ...
  loadOut(); // //p.addEventListener('canplaythrough', audioLoadedHandler);
...
function start(r) {
  ...
  if (r !== null) {
    ...
    p.currentTime = x; //p.volume = 0;

    startPlaying(); // $("#playOverlay").fadeOut(1000);
...
function check() {
  ...
  if (masterVolume !== m && m) {
    setMaster(m, 2000); // console.log("setting master: " + m);
  } // s = Date.now();
  ...
  $.ajax({
    ...
    success: function success(result) {
      $("#check").html(result + d);
      var processTime = (startTime - Date.now()) / 1000; // console.log("TIME", result, p.currentTime);

      adjustTime(result - processTime, d); // start(result);
    }
  });
...
function animateAudio(volume, time) {
  var callback = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : function () {
    console.log("END VOLUME ANIMATION");
  };
  $("#audio").animate({
    volume: volume / 100
  }, time, callback);
...
function getMaster(id) {
  ...
  $.ajax({
    ...
    success: function success(result) {
      ...
      res = parseInt(result); // console.log(result);
    }
  });
  return res;
}
...
function setTrack(data) {
  console.log(data);
  data = $.parseJSON(data);
  console.log(data); //p.pause();

  currentSRC = data.src;
  currentId = data.id; // alert(data)

  $("#track").val(currentSRC);
  $("#trackH3").html(currentSRC); // $("#audio").attr("src", "http://francescoflaviomartinelli.com/SynchServerTime/Tracce/" + currentSRC);
